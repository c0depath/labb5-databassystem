...
DECLARE
	...
	V_KTNR KONTO.KTNR%TYPE; 
	V_REGDATUM KONTO.REGDATUM%TYPE; 
	V_SALDO KONTO.SALDO%TYPE; 
BEGIN 
SELECT KNR, KTNR, REGDATUM, SALDO
INTO V_KNR, V_KTNR, V_REGDATUM, V_SALDO
FROM KONTO
WHERE KNR = '123';
DBMS_OUTPUT.PUT_LINE('Kundnr: ' || V_KNR || ' Kontonummer: ' || V_KTNR || ' Regdatum: ' || V_REGDATUM || ' Saldo: ' || V_SALDO);
DO_UTTAG('630214-1213', 123, 1200);
END;

DECLARE
    V_RADNR UTTAG.RADNR%TYPE;
	V_PNR UTTAG.PNR%TYPE; 
	V_KNR UTTAG.KNR%TYPE; 
	V_BELOPP UTTAG.BELOPP%TYPE;
	V_DATUM UTTAG.DATUM%TYPE;
BEGIN 
SELECT RADNR, PNR, KNR, BELOPP, DATUM
INTO V_RADNR, V_PNR, V_KNR, V_BELOPP, V_DATUM
FROM UTTAG
WHERE KNR = '123';
DBMS_OUTPUT.PUT_LINE('Radnr: ' || V_RADNR || ' Personnummer: ' || V_PNR || ' Kundnummer: ' || V_KNR || ' Belopp: ' || V_BELOPP || ' Datum: ' || V_DATUM);
DO_UTTAG('630214-1213', 123, 1200);
END;
...
BEGIN 
DO_UTTAG('930112-1105', 123, 50000000000);
END;

BEGIN 
DO_UTTAG('930112-1105', 123, -59000000000);
END;
